defmodule ScenicWidgets.SideNav do
  ...
  def render_file_tree(graph, frame, tree) when is_list(tree) do
    # Start at level 0, as this is the top level item, we increment it on the first iteration so the first index is 1
    {final_graph, _final_offsets} = do_render_file_tree(graph, frame, tree, {0, 0})
    final_graph
  end
  ...
  def do_render_file_tree(
        graph,
        outer_frame,
        [{:open_node, label, index, sub_tree} = item | rest],
        {x_offset, y_offset}
      ) do
    ...
    # add another level of offsets, because we just went down a level
    {opened_node_graph, {_sub_x_offset, new_y_offset}} =
      do_render_file_tree(new_graph, outer_frame, sub_tree, {x_offset + 1, y_offset + 1})
