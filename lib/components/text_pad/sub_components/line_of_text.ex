defmodule ScenicWidgets.TextPad.LineOfText do
  ...
  def validate(
        %{
          ...
          frame: %Widgex.Frame{} = _frame,
          text: text,
          # TODO this sucks so bad but we need it to get the text back later using GenServer.call
          name: name,
          theme: theme
        } = args
      )
      when is_integer(line_num) and line_num >= 1 and (is_bitstring(text) or is_nil(text)) do
    {:ok, args}
  end
    ...
    init_graph =
      if is_nil(text) or text == "" do
        ...
        |> Scenic.Primitives.text(text,
          ...
          translate: {args.frame.pin.x, args.frame.pin.y + ascent}
        )
      end
